...
import { utils, ethers as ethers$1, Contract } from 'ethers';
...
var getSafeConfigs = (fileName) => {
  return JSON.parse(readFileSync(fileName, GNOSIS_SAFE_FILE_ENCODING));
};
...
var logError = (message, includeErrorText = true) => console.log(chalk.red(includeErrorText ? `ERROR: ${message}` : message));
var logWarning = (message) => console.log(chalk.yellow(`WARNING: ${message}`));
var logSuccess = (message) => console.log(chalk.green(`${message}`));
var configExist = (fileName) => fs.existsSync(fileName);
var getConfig = (fileName) => JSON.parse(fs.readFileSync(fileName, "utf-8"));
...
var getWallet = (index) => ethers.Wallet.fromMnemonic(process.env.MNEMONIC || "", `m/44'/60'/0'/0/${index}`);
...
var executeGnosisTransactions = async (hre, network, gnosisConfig, transactions) => {
  ...
  const safeSdk = await Safe.create({ ethAdapter, safeAddress, ...!!contractNetworks && { contractNetworks } });
...
var getDefaultConfig_default = async (taskArgs, hre) => {
  ...
  const configByNetwork = await Promise.all(
    networks.map(async (network) => {
      const provider = getProvider(hre, network);
      console.log();
      const endpoint = new ethers$1.Contract(getEndpointAddress(network), ENDPOINT_ABI2, provider);
      ...
      const messagingLibrary = new ethers$1.Contract(sendLibraryAddress, MESSAGING_LIBRARY_ABI2, provider);
...
var getRemoteConfig = async (remoteNetwork, sendLibrary, receiveLibrary, contractAddress) => {
  ...
  return {
    ...
    relayer: appConfig.relayer != ethers$1.constants.AddressZero ? appConfig.relayer : defaultAppConfig.relayer,
    ...
    oracle: appConfig.oracle != ethers$1.constants.AddressZero ? appConfig.oracle : defaultAppConfig.oracle
  };
};
...
var getVersions = async (endpoint, contractAddress, provider) => {
  ...
  const sendLibrary = new ethers$1.Contract(sendLibraryAddress, MESSAGING_LIBRARY_ABI, provider);
  ...
  const receiveLibrary = new ethers$1.Contract(receiveLibraryAddress, MESSAGING_LIBRARY_ABI, provider);
  return {
    sendVersion,
    receiveVersion,
    sendLibrary,
...
var generateAppConfig = async (hre, configs, checkConnectionFunctionFragment, name) => {
  ...
  return networks.reduce(async (acc, network) => {
    ...
    const endpoint = new ethers$1.Contract(endpointAddress, ENDPOINT_ABI, provider);
...
// src/index.ts
task("setConfig", "sets Send and Receive Messaging Library versions and a custom application config for contracts implementing ILayerZeroUserApplicationConfig interface", setConfig_default).addParam("configPath", "the application config file path").addOptionalParam("name", "name of the deployed contracts. Should be specified if the same contract deployed on different chains and the deployment information is located in the deployments folder").addOptionalParam("address", "address of the deployed contracts. Should be specified if the contract address is the same on all chains").addOptionalParam("gnosisConfigPath", "the path to a file with Gnosis config. If specified, the transactions will be sent to Gnosis").addOptionalParam("gasLimit", "override execution gasLimit");
task("getDefaultConfig", "outputs the default Send and Receive Messaging Library versions and the default application config", getDefaultConfig_default).addParam("networks", "comma separated list of networks");
task("generateAppConfig", "generates the config mesh for a User Application", generateAppConfigAction).addParam("networks", "comma separated list of networks").addOptionalParam("name", "name of the deployed contract. Should be specified only if the deployment information is located in the deployments folder").addOptionalParam("outputFileName", "the path to the output file", "./constants/defaultConfig.json", types.string).addOptionalParam("checkConnectionFunctionFragment", "the checkConnection function fragment", LZ_APP_TRUSTED_REMOTE_LOOKUP_FUNCTION_FRAGMENT, types.string);
task("getConfig", "outputs the application's Send and Receive Messaging Library versions and the config for remote networks", getConfig_default).addParam("remoteNetworks", "comma separated list of remote networks").addOptionalParam("name", "name of the deployed contract. Should be specified only if the deployment information is located in the deployments folder").addOptionalParam("address", "the contract address");
task("checkWireAllConfig", "", checkWireAllConfig_default).addParam("e", "the environment ie: mainnet, testnet or sandbox").addFlag("u", "show use custom adapter params").addFlag("t", "show trusted remote lookup").addFlag("m", "show min destination gas lookup").addParam("chains", "comma separated list of networks").addOptionalParam("contract", "name of contract").addOptionalParam("addresses", "addresses of contracts in same order as chains").addOptionalParam("proxyContract", "name of proxy contract").addOptionalParam("proxyChain", "name of proxy chain");
task("wireAll", "", wireAll_default).addParam("e", "the environment ie: mainnet, testnet or sandbox").addOptionalParam("noPrompt", "no prompt", false, types.boolean).addOptionalParam("configPath", "Optional config path. Default: ./constants/wireUpConfig.json", "./constants/wireUpConfig.json", types.string).addOptionalParam("n", "send to gnosis", false, types.boolean).addOptionalParam("gasLimit", "override execution gasLimit").addOptionalParam("gnosisConfigPath", "Optional config path. Default: ./constants/gnosisConfig.json", "./constants/gnosisConfig.json", types.string);
